...
class RP4WP_Hook_Link_Related_Screen extends RP4WP_Hook {
	...
	public function run() {
		...
		// Add Page
		$screen_hook = add_submenu_page( null, 'Link_Related_Screen', 'Link_Related_Screen', 'edit_posts', 'rp4wp_link_related', array(
			$this,
			'content'
		) );
	...
	private function handle_create_link() {
		...
		if ( isset( $_GET['rp4wp_create_link'] ) && isset( $_GET['rp4wp_parent'] ) ) {
			...
			// Create link
			$post_link_manager->add( $parent, $_GET['rp4wp_create_link'], $parent_post_type, false, true );
		...
		if ( isset( $_POST['rp4wp_bulk'] ) && isset( $_GET['rp4wp_parent'] ) ) {
			...
			if ( count( $_POST['rp4wp_bulk'] ) > 0 ) {
				foreach ( $_POST['rp4wp_bulk'] as $bulk_post ) {
					...
					// Create link
					$post_link_manager->add( $parent, $bulk_post, $parent_post_type, false, true );
		...
		<div class="wrap">
			<h2>
				...
				<a href="<?php echo $cancel_url; ?>"
				   class="add-new-h2"><?php _e( 'Cancel linking', 'related-posts-for-wp' ); ?></a>
			</h2>
			...
			<form id="sp-list-table-form" method="post">
				...
				$list_table = new RP4WP_Link_Related_Table( $parent_post_type );
